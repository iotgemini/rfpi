...
Description: it check if the peri answer with the settings of the GPIO
...
echo '<br><p>Reading settings GPIO ....</p>';
...
if( ($counter < $max_counter)){ //if it is not in time out
		...
		if($data[0]==='O' && $data[1]==='K'){ //if there is no answer about the packet then will send a command again
			...
			if($data[0]==='N' && $data[1]==='O' && $data[2]==='T' && $data[3]==='X'){
				...
				if($address_peri === $address_peri_from_reply /*&& $id_packet === $id_packet_reply*/){ //if it is the packet wanted, thus can save it into the DB
					$gpio0_setting = $data_radio[11]; 
					$gpio1_setting = $data_radio[13];
					header('Location: ./gpio_functions.php?address_peri='.$address_peri.'&position_id='.$position_id.'&gpio0_setting='.$gpio0_setting.'&gpio1_setting='.$gpio1_setting);
					$reload_this_page = 0;
				}else{ //it retry 
					//will make a retry
					...
					if($cont_retry<$max_cont_retry+1){
						...
						echo 'setTimeout("';
						echo "location.href = './cmd_get_gpio_setting.php?address_peri=".$address_peri."&position_id=".$position_id."&counter=" . $counter . "&cont_retry=".$cont_retry."&$redirect_page=".$redirect_page."';";
						echo '", 10); ';
						echo '</script>';
					}else{
						...
						echo '<script type="text/javascript">';
						echo "setTimeout('";
						echo 'location.href = "./gpio_functions.php?address_peri='.$address_peri.'&position_id='.$position_id.'";';
						echo "', 3000);";
						echo '</script>';
					}
	...
	if($reload_this_page == 1){
		echo '<script type="text/javascript">';
		echo 'setTimeout("';
		echo "location.href = './read_fifo_gpio_setting.php?address_peri=".$address_peri."&position_id=".$position_id."&counter=" . $counter ."&cont_retry=" . $cont_retry . "&redirect_page=".$redirect_page."';";
		echo '", 25); ';
		echo '</script>';
	}
	...
	$cont_retry++;
	if($cont_retry<$max_cont_retry+1){
		//header('Location: ./cmd_get_gpio_setting.php?address_peri='.$address_peri.'&counter='.$counter.'&cont_retry='.$cont_retry.'&redirect_page='.$redirect_page);
		echo '<br><p>';
		echo 'Retry '; echo $cont_retry; echo ' of '; echo $max_cont_retry;
		echo '</p>';
