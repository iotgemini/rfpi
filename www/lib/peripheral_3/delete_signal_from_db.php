...
Description: it delete a line into the signal database of the current peripheral
...
if(file_exists("./db/".$address_peri."_db_infrared_signals.txt")){
	...
	//finding the line and writing on it the following line
	$handleFileR = fopen("./db/".$address_peri."_db_infrared_signals.backup", 'r');
	$handleFileW = fopen("./db/".$address_peri."_db_infrared_signals.txt", 'w');
	$i=0; $t=0;
	while(feof($handleFileR)!==TRUE){ 
		if($data=fgets($handleFileR)){
			if($i!=$line_position)
				fwrite($handleFileW,$data);
