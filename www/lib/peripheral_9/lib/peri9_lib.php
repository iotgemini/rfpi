...
Description: it is the library with all useful function to display data picked up by Peri 9
...
//-------------------------------BEGIN LIST AVAILABLE FORMULAS----------------------------------//

//-------------------------------END LIST AVAILABLE FORMULAS----------------------------------//
/*
	INPUT_FORMULA_	= 1		->		Temperature Pyrometer
	INPUT_FORMULA_	= 2		->		Voltage TVDC
	INPUT_FORMULA_	= 3		->		xxx
	INPUT_FORMULA_	= 4		->		Voltage 0-10V for IN0
	INPUT_FORMULA_	= 5		->		Voltage 0-10V for IN1
	INPUT_FORMULA_	= 6		->		Voltage 0-10V for IN2
	INPUT_FORMULA_	= 7		->		Voltage 0-10V for IN3
	INPUT_FORMULA_	= 8		->		Heater Running (YES/NO) (input 4)
	INPUT_FORMULA_	= 9		->		Heater Power kW
	INPUT_FORMULA_	= 10	->		Production speed mt/min
	INPUT_FORMULA_	= 11	->		Percentage of the Heater Power (Potentiometer Reference Set)
	INPUT_FORMULA_	= 12	->		Percentage of the Heater Production (Potentiometer Reference Set)
	INPUT_FORMULA_	= 13	->		Temperature NTC on IN2 (fatto per le piscine)
	INPUT_FORMULA_	= 14	->		Temperature NTC on IN3 (fatto per le piscine)
	INPUT_FORMULA_	= 15	->		Flow Meter (fatto per le piscine)
	INPUT_FORMULA_	= 16	->		Recording (YES/NO) (fatto per le piscine)
	
	else 			=		->		Voltage between 0-10V
...
//	function return_str_with_formula_name($formula_to_show);
	
//	function return_num_with_formula_set($data_to_convert, $formula_to_show);

//	function return_str_with_formula_set($data_to_convert, $formula_to_show);
	
//	function temperature_MCP9701_from_8bit_value_peri9($ADC_8bit_value); 	//return the temperature in °C

//	function the_8bit_value_from_temperature_MCP9701_peri9($tempearure); 	//return the byte of the temperature

//	function voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value);			//return the value between 0V and 10V from the byte readed by the ADC

//	function str_voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value);		//return the string with the value between 0V and 10V from the byte readed by the ADC

//	function value_8bit_from_voltage_0to10V_peri9($voltage_0to10_value);	//return a byte from a value between 0V and 10V

//	function save_data_peri9($array_data,$lenght_array,$address);			//save all data got from the peri into a file named with its address

//	function voltage_TVDC_peri9($ADC_8bit_value);							//return the integer of the TVDC

//	function str_voltage_TVDC_peri9($ADC_8bit_value);								//return a string with the value of the TVDC

//	function read_data_peri9($address, $file_path);							//return an array with alla data readed from the file. The first data is the number of data count

//	function print_the_file_data($address_peri, $formula_to_show,$file_data_path);			//print all data saved into the file of the peri

//	function print_only_the_last_4data_from_the_file($address_peri, $array_input_formula_to_show,$file_data_path); //print the last 4 datas from the file

// 	function power_KW_peri9($ADC_8bit_value);								//return the power refered to 0-10V

//	function production_mt_min_peri9($ADC_8bit_value);						//return the production speed refered to 0-10V

//	function rif_power_percent_peri9($ADC_8bit_value);						//return the 0-100% took from the potentiometer

//	function rif_production_percent_peri9($ADC_8bit_value);					//return the 0-100% took from the potentiometer

//draw a chart using the library Chart.min.js :
//	function draw_chart_of_data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path, $sem_dislay_data0, $sem_dislay_data1, $sem_dislay_data2, $sem_dislay_data3, $sem_dislay_data4, $sem_dislay_time, $sem_dislay_date);	


//	print_7columns_of_data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path); //it also print date and time
...
//define("DIRECTORY_IMG_PERI_9", "./lib/peripheral_9/img/"); 		//where all pictures are kept
//define("DIRECTORY_CSS_PERI_9", "./lib/peripheral_9/css/"); 		//where all pictures are kept

define("DIRECTORY_IMG_PERI_9", "/img/peripheral/"); 		//where all default pictures for any peripheral are kept
define("DIRECTORY_CSS_PERI_9", "/css/"); 					//where all default style for any peripheral are kept


define("MAX_TEMPERATURE_MCP9701", 128);
define("MIN_TEMPERATURE_MCP9701", 0);

define("MAX_NUM_OF_DATA_PERI9", 1000);
define("NUM_CHARACTERS_IN_A_LINE_OF_THE_FILE_PERI9", 200);
...
//-------------------------------END INCLUDE CSS----------------------------------//
	...
	$lang_temp_pyrometer_peri9 = "Pyrometer";
	$lang_heater_running_peri9 = "Heater Running";
	$lang_power_peri9 = "Power";
	$lang_production_peri9 = "Production";
	$lang_rif_power_peri9 = "Power Ref.";
	$lang_rif_production_peri9 = "Production Ref.";
	$lang_temperatura_interna_peri9 = "IN2 Temp. (&nbsp&deg;C&nbsp)";
	$lang_temperatura_esterna_peri9 = "IN3 Temp. (&nbsp&deg;C&nbsp)";
	$lang_flow_meter_peri9 = "Flow (&nbspL&#47;Min&nbsp)";
	$lang_recording_peri9 = "Rec.";
	if($_SESSION["language"]=="IT"){
		$lang_temp_pyrometer_peri9 = "Pirometro";
		$lang_heater_running_peri9 = "Riscaldatore in marcia";
	}else if($_SESSION["language"]=="FR"){
		$lang_temp_pyrometer_peri9 = "Pyrom&egrave;tre";
		$lang_heater_running_peri9 = "Le chauffage est en cours d&rsquo;ex&eacute;cution";
	}else if($_SESSION["language"]=="SP"){
		$lang_temp_pyrometer_peri9 = "Pir&oacute;metro";
		$lang_heater_running_peri9 = "Calentador est&aacute; funcionando";
...
function return_str_with_formula_name($formula_to_show){
	global $lang_temp_pyrometer_peri9, $lang_heater_running_peri9,$lang_power_peri9,$lang_production_peri9,$lang_rif_power_peri9,$lang_rif_production_peri9;
	
	$str_to_return = "";
			if($formula_to_show==1){
				$str_to_return = $lang_temp_pyrometer_peri9;
			}else if($formula_to_show==2){
				$str_to_return = "TVDC";
			}else if($formula_to_show==4){
				$str_to_return = "IN0";
			}else if($formula_to_show==5){
				$str_to_return = "IN1";
			}else if($formula_to_show==6){
				$str_to_return = "IN2";
			}else if($formula_to_show==7){
				$str_to_return = "IN3";
			}else if($formula_to_show==8){
				$str_to_return = $lang_heater_running_peri9;
			
			}else if($formula_to_show==9){
				$str_to_return = $lang_power_peri9;			
			}else if($formula_to_show==10){
				$str_to_return = $lang_production_peri9;	
			
			}else if($formula_to_show==11){
				$str_to_return = $lang_rif_power_peri9;			
			}else if($formula_to_show==12){
				$str_to_return = $lang_rif_production_peri9;	
			
			}else if($formula_to_show==13){ //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				$str_to_return = "IN2 Temp. (&nbsp&deg;C&nbsp)"; //$lang_temperatura_interna_peri9;
			}else if($formula_to_show==14){ //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori	
				$str_to_return = "IN3 Temp. (&nbsp&deg;C&nbsp)"; //$lang_temperatura_esterna_peri9;
			}else if($formula_to_show==15){ //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori	
				$str_to_return = "Flow (&nbspL&#47;Min&nbsp)";//$lang_flow_meter_peri9;
			}else if($formula_to_show==16){ //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori	
				$str_to_return = "Rec."; //$lang_recording_peri9;
...
function return_num_with_formula_set($data_to_convert, $formula_to_show){
	$num_to_return = 0; 
			if($formula_to_show==1){ 
				$num_to_return = temperature_pyrometer_peri9($data_to_convert); 
				
			}else if($formula_to_show==2){
				$num_to_return = voltage_TVDC_peri9($data_to_convert); 

			}else if($formula_to_show==4){
				$num_to_return =  voltage_0to10V_from_8bit_value_peri9($data_to_convert);
				...
				$num_to_return =  power_KW_peri9($data_to_convert);
				
			}else if($formula_to_show==10){ 
				$num_to_return =  production_mt_min_peri9($data_to_convert);
			
			}else if($formula_to_show==11){ 
				$num_to_return =  rif_power_percent_peri9($data_to_convert);

			}else if($formula_to_show==12){ 
				$num_to_return =  rif_production_percent_peri9($data_to_convert);
			
			}else if($formula_to_show==13){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				$num_to_return =  temperatura_int_ext_peri9($data_to_convert);

			}else if($formula_to_show==14){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				$num_to_return =  temperatura_int_ext_peri9($data_to_convert);

			}else if($formula_to_show==15){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				$num_to_return =  flow_meter_peri9($data_to_convert);
...
function return_str_with_formula_set($data_to_convert, $formula_to_show, $only_data){
	$str_to_return = ""; 
			if($formula_to_show==1){ 
				$str_to_return .= str_temperature_pyrometer_peri9($data_to_convert,$only_data); 
				//if($cont+1 == $cont_temperature_found) echo $lang_the_last;
				//echo '&nbsp&#176C&nbsp'; //°C
				if($only_data==0) $str_to_return .= "<br>";

			}else if($formula_to_show==2){
				$str_to_return .= str_voltage_TVDC_peri9($data_to_convert,$only_data); 
				$str_to_return .= '&nbspV&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";

			}else if($formula_to_show==4){
				if($only_data==0) $str_to_return .= '&nbspIN0= ';
				$str_to_return .=  str_voltage_0to10V_from_8bit_value_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspV&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
				
			}else if($formula_to_show==5){
				if($only_data==0) $str_to_return .= '&nbspIN1= ';
				$str_to_return .=  str_voltage_0to10V_from_8bit_value_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspV&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
				
			}else if($formula_to_show==6){ 
				if($only_data==0) $str_to_return .= '&nbspIN2= ';
				$str_to_return .=  str_voltage_0to10V_from_8bit_value_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspV&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
			
			}else if($formula_to_show==7){
				if($only_data==0) $str_to_return .= '&nbspIN3= ';
				$str_to_return .=  str_voltage_0to10V_from_8bit_value_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspV&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
			
			}else if($formula_to_show==8){
				if($only_data==0) $str_to_return .= '&nbspHeater Running= ';
				if($data_to_convert==1)
					$str_to_return .= 'YES'; //
				else
					$str_to_return .= 'NO'; //
				if($only_data==0) $str_to_return .= "<br>";
				...
				if($only_data==0) $str_to_return .= '&nbspIN0= ';
				$str_to_return .=  str_power_KW_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspKW&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
				
			}else if($formula_to_show==10){ 
				if($only_data==0) $str_to_return .= '&nbspIN1= ';
				$str_to_return .=  str_production_mt_min_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbspmt&frasl;min&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
			
			}else if($formula_to_show==11){ 
				if($only_data==0) $str_to_return .= '&nbspIN0= ';
				$str_to_return .=  str_rif_power_percent_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbsp%&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
				
			}else if($formula_to_show==12){ 
				if($only_data==0) $str_to_return .= '&nbspIN1= ';
				$str_to_return .=  str_rif_production_percent_peri9($data_to_convert,$only_data);
				$str_to_return .= '&nbsp%&nbsp'; //V
				if($only_data==0) $str_to_return .= "<br>";
			
			}else if($formula_to_show==13){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				if($only_data==0) $str_to_return .= '&nbspT. Int.= ';
				$str_to_return .=  str_temperatura_int_ext_peri9($data_to_convert,$only_data);
				//$str_to_return .= '&nbsp&degC&nbsp'; //°C
				if($only_data==0) $str_to_return .= "<br>";
			}else if($formula_to_show==14){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				if($only_data==0) $str_to_return .= '&nbspT. Ext.= ';
				$str_to_return .=  str_temperatura_int_ext_peri9($data_to_convert,$only_data);
				//$str_to_return .= '&nbsp&deg;C&nbsp'; //°C
				if($only_data==0) $str_to_return .= "<br>";
			}else if($formula_to_show==15){  //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				if($only_data==0) $str_to_return .= '&nbspFlow= ';
				$str_to_return .=  str_flow_meter_peri9($data_to_convert,$only_data);
				//$str_to_return .= '&nbspL&#47;Min&nbsp'; //L/Min
				if($only_data==0) $str_to_return .= "<br>";
			}else if($formula_to_show==16){ //fatto per lo scatolotto che registra il Delta Temperature su raffreddamento tristori
				if($only_data==0) $str_to_return .= '&nbspRecording= ';
				if($data_to_convert==1)
					$str_to_return .= 'YES'; //
				else
					$str_to_return .= 'NO'; //
...
function temperature_MCP9701_from_8bit_value_peri9($ADC_8bit_value){
...
function the_8bit_value_from_temperature_MCP9701_peri9($tempearure){
	$value_8bit = (($tempearure * 256) / MAX_TEMPERATURE_MCP9701)+MIN_TEMPERATURE_MCP9701; 
	//$value_8bit = ceil($value_8bit);
	return $value_8bit;
}

function voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value){
...
function str_voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value){
	$voltage="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$voltage_int = voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value);
...
function value_8bit_from_voltage_0to10V_peri9($voltage_0to10_value){
...
function temperature_pyrometer_peri9($ADC_8bit_value){
	$temperature = (($ADC_8bit_value * (FORMULA_1_MAX_VALUE-FORMULA_1_MIN_VALUE) ) / 255)+FORMULA_1_MIN_VALUE;
		
	return $temperature;
}


function str_temperature_pyrometer_peri9($ADC_8bit_value){
	$temperature = temperature_pyrometer_peri9($ADC_8bit_value);
...
function read_data_peri9($address, $file_path111){
	   // echo $address; echo "<br>";
		//$file_path = "./lib/peripheral_9/data/" . $address . "_data.txt"; 
		//echo $file_path111;  echo "<br>";
		//$file_path = ".././data/". $address ."_data.txt";
		$array_data = array();
		$cont_lines=0;
		$cont_data_found=0;
		array_push($array_data, strval($cont_data_found));
		if(file_exists($file_path111)===TRUE){ //echo "ciao";
			$handle_file = fopen($file_path111, 'r');
			while(feof($handle_file)!==TRUE){ // && $cont_lines<30){ 
				$line_file=fgets($handle_file, NUM_CHARACTERS_IN_A_LINE_OF_THE_FILE_PERI9);
				
				if (strpos($line_file,'TEMPERATURE') !== false) {
...
function save_data_peri9($array_data,$lenght_array,$address){
	...
	if(file_exists($file_path)===TRUE){ //echo "ciao";
		...
		while(feof($handle_file)!==TRUE){ // && $cont_lines<MAX_NUM_OF_DATA_PERI9){ 
			$line_file=fgets($handle_file, NUM_CHARACTERS_IN_A_LINE_OF_THE_FILE_PERI9);
	...
	if($cont_lines > MAX_NUM_OF_DATA_PERI9){
		$data_to_write_into_file="";
		$i = $cont_lines - MAX_NUM_OF_DATA_PERI9;
		$j=0;
		while($j<MAX_NUM_OF_DATA_PERI9){
...
function voltage_TVDC_peri9($ADC_8bit_value){
	$voltage = $ADC_8bit_value; 
	$voltage = (($ADC_8bit_value * 450 ) / 256); 
	//$voltage = ceil($voltage);
	return $voltage;
}


function str_voltage_TVDC_peri9($ADC_8bit_value){
	$voltage="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$voltage_int = voltage_TVDC_peri9($ADC_8bit_value);
	$voltage_int = $voltage_int * 100;
	$voltage_int = ceil($voltage_int);
	$voltage_int = $voltage_int / 100;
	$voltage = number_format($voltage_int, 0, '.', ''); 
	return $voltage;
}


function print_the_file_data($address_peri, $formula_to_show, $file_data_path){
//$array_temperature_pyrometer = array();
		$array_temperature_pyrometer = read_data_peri9($address_peri, $file_data_path);
		$cont_temperature_found = intval ($array_temperature_pyrometer[0], 10);
		//$cont_temperature_found = read_data_peri9($address_peri, $array_temperature_pyrometer);
		//echo "last: <br><br>";
		$cont = $cont_temperature_found;
		//echo $cont_temperature_found;	echo "<br>";
		while($cont>0){
			echo return_str_with_formula_set($array_temperature_pyrometer[$cont], $formula_to_show);
			echo "<br>";
			$cont--;
		}
...
/*function print_only_the_last_4data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path){
		 $num_input_saved_into_file = 4;
		 
//$array_temperature_pyrometer = array();
		$array_temperature_pyrometer = read_data_peri9($address_peri, $file_data_path); //echo $file_data_path;
		$cont_temperature_found = intval ($array_temperature_pyrometer[0], 10);
		//$cont_temperature_found = read_data_peri9($address_peri, $array_temperature_pyrometer);
		//echo "last: <br><br>";
		$cont = $cont_temperature_found;
		...
		while($cont>0){
			...
			if($num_input_saved_into_file>=0){
				
				
				echo return_str_with_formula_set($array_temperature_pyrometer[$cont], $array_input_formula_to_show[$num_input_saved_into_file]);
				//echo "<br>";
				//echo "<br>"; echo $array_input_formula_to_show[$num_input_saved_into_file]; echo "<br>";
				
				$num_input_saved_into_file--;
				$cont--;
			}else{
				$cont=0;
			}
...
function print_only_the_last_4data_from_the_file_peri9($address_peri, $array_input_formula_to_show, $file_data_path){
		...
		$cont_position=0;
		$array_last_5_data = array();
		while($cont_position<5){
			array_push($array_last_5_data, "");
			$cont_position++;
		}
		$cont_position = 0;
		
		//array_push($array_data, strval($cont_data_found));
		if(file_exists($file_data_path)===TRUE){ //echo "ciao";
			$handle_file = fopen($file_data_path, 'r');
			while(feof($handle_file)!==TRUE){ // && $cont_lines<30){ 
				$line_file=fgets($handle_file, NUM_CHARACTERS_IN_A_LINE_OF_THE_FILE_PERI9);
				...
				if (strpos($line_file,'TEMPERATURE') !== false) {
					...
					if($cont_position<5){
						$array_last_5_data[$cont_position] = $str_sub_temp;
						$cont_position++;
					}else{
						$cont_position=4;
						while($cont_position>0){
							$array_last_5_data[$cont_position - 1] = $array_last_5_data[$cont_position];
							$cont_position--;
						}
						$cont_position = 4;
						$array_last_5_data[$cont_position] = $str_sub_temp;
	...
	$cont = 4;
		while($cont>=0){
			...
			if($num_input_saved_into_file>=0){
				...
				echo return_str_with_formula_set($array_last_5_data[$cont], $array_input_formula_to_show[$num_input_saved_into_file]);
...
function	print_5columns_of_data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path){
	

	$num_input_saved_into_file = 5;

		 
		echo '<table class="table_peripheral" border=1>';
		echo '<tr class="table_title_field_line">'; 
		
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[6]) . '</td>'; 
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[5]) . '</td>'; 
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[4]) . '</td>';
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[3]) . '</td>';
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[2]) . '</td>';
		
		echo '</td>'; 
		echo '</tr>';
		
		$array_data = read_data_peri9($address_peri, $file_data_path); //echo $file_data_path;
		$cont_data_found = intval ($array_data[0], 10);
		$cont = $cont_data_found-1;

		$cont_num_input = $num_input_saved_into_file-1;
		while($cont>=0){

			echo '<tr class="table_line_even" >';
			
			while($cont_num_input>=0){
				
				echo '<td class="td_peripheral" align=center >';
				
				echo return_str_with_formula_set($array_data[$cont], $array_input_formula_to_show[$cont_num_input+2], 1);
				echo "<br>";
				//echo "<br>"; echo $array_input_formula_to_show[$num_input_saved_into_file]; echo "<br>";
...
function save_num_data_peri9($sem_status_send_data_after_stop, $num_data_send, $address_peri){
	
	if(!is_numeric($sem_status_send_data_after_stop)) $sem_status_send_data_after_stop = "0";
	if(!is_numeric($num_data_send)) $num_data_send = "1";
	
	$file_path = "./data/". $address_peri ."_conf_data.txt";
	
	$myfile = fopen($file_path , "w");

	$data_to_write_into_file = $sem_status_send_data_after_stop . " " . $num_data_send . "\n";

	fwrite($myfile, $data_to_write_into_file);
...
function power_KW_peri9($ADC_8bit_value){
	
	$power = $ADC_8bit_value; 
	$power = ($ADC_8bit_value * ((FORMULA_9_MAX_VALUE/10)-FORMULA_9_MIN_VALUE)) ; 
...
function str_power_KW_peri9($ADC_8bit_value){
	$power="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$power_int = power_KW_peri9($ADC_8bit_value);
	$power_int = $power_int * 100;
	$power_int = ceil($power_int);
	$power_int = $power_int / 100;
	$power = number_format($power_int, 2, '.', ''); 
	return $power;
}


function production_mt_min_peri9($ADC_8bit_value){
	
	$production = $ADC_8bit_value; 
	$production = ($ADC_8bit_value * (FORMULA_10_MAX_VALUE-FORMULA_10_MIN_VALUE)) ; 
...
function str_production_mt_min_peri9($ADC_8bit_value){
	$production="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$production_int = production_mt_min_peri9($ADC_8bit_value);
	$production_int = $production_int * 100;
	$production_int = ceil($production_int);
	$production_int = $production_int / 100;
	$production = number_format($production_int, 2, '.', ''); 
	return $production;
}



function rif_power_percent_peri9($ADC_8bit_value){
	
	$power = $ADC_8bit_value; 
	$power = ($ADC_8bit_value * 10) ; 
	$power = $power / 256; //divide by 256 because is the maximum value on 8bit
	//$power = ceil($power);
	return $power;
...
function str_rif_power_percent_peri9($ADC_8bit_value){
	$power="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$power_int = rif_power_percent_peri9($ADC_8bit_value);
	$power_int = $power_int * 100;
	$power_int = ceil($power_int);
	$power_int = $power_int / 100;
	$power = number_format($power_int, 0, '.', ''); 
	return $power;
}


function rif_production_percent_peri9($ADC_8bit_value){
	
	$production = $ADC_8bit_value; 
	$production = ($ADC_8bit_value * 10) ; 
	$production = $production / 256; //divide by 256 because is the maximum value on 8bit
	//$production = ceil($production);
	return $production;
...
function str_rif_production_percent_peri9($ADC_8bit_value){
	$production="";
	//$voltage=number_format((float)strval(voltage_0to10V_from_8bit_value_peri9($ADC_8bit_value)), 0, '.', ''); 
	$production_int = rif_production_percent_peri9($ADC_8bit_value);
	$production_int = $production_int * 100;
	$production_int = ceil($production_int);
	$production_int = $production_int / 100;
	$production = number_format($production_int, 0, '.', ''); 
	return $production;
}



function str_temperatura_int_ext_peri9($ADC_8bit_value){
	$value_temp = $ADC_8bit_value & 127;
	if(($ADC_8bit_value & 128) == 128){
		$value_temp += 0.5;
	}
	$temperatura = "";
	$temperatura .= $value_temp;
	
	return $temperatura;
}

function str_flow_meter_peri9($ADC_8bit_value){
	$flow_meter = "";
	//$flow_meter .= ($ADC_8bit_value/10);
	$temp1 = $ADC_8bit_value;
	//$temp1 = $temp1 / 3;
	$temp1 = $temp1 / 10;
	$flow_meter .= $temp1;
	return $flow_meter;
...
function draw_chart_of_data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path, $id_name_canvas0, $size_px_canvas, $sem_dislay_data0, $sem_dislay_data1, $sem_dislay_data2, $sem_dislay_data3, $sem_dislay_data4, $sem_dislay_time, $sem_dislay_date){
	...
	$array_date = array();
		...
		$cont_lines=0;
		$cont_data_found=0;
		if(file_exists($file_data_path)===TRUE){ 
			$handle_file = fopen($file_data_path, 'r');
			$sem_all_data_got=0;
			while(feof($handle_file)!==TRUE){ 
				$line_file=fgets($handle_file, NUM_CHARACTERS_IN_A_LINE_OF_THE_FILE_PERI9);
				//echo $line_file; echo "<br>";
				
				if (strpos($line_file,'DATA0') !== false) { 
					$position1 = strpos($line_file,'DATA0=') + 6;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_data0, $str_sub_temp);
					//$array_data0[$cont_lines] = (int)str_replace(' ', '', $str_sub_temp);
					$sem_all_data_got++;
					//echo "data0="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'DATA1') !== false) {
					$position1 = strpos($line_file,'DATA1=') + 6;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_data1, $str_sub_temp);
					$sem_all_data_got++;
					//echo "data1="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'DATA2') !== false) {
					$position1 = strpos($line_file,'DATA2=') + 6;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_data2, $str_sub_temp);
					$sem_all_data_got++;
					//echo "data2="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'DATA3') !== false) {  
					$position1 = strpos($line_file,'DATA3=') + 6;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_data3, $str_sub_temp);
					$sem_all_data_got++;
					//echo "data3="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'DATA4') !== false) { 
					$position1 = strpos($line_file,'DATA4=') + 6;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_data4, $str_sub_temp);
					$sem_all_data_got++;
					//echo "data4="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'TIME') !== false) { 
					$position1 = strpos($line_file,'TIME=') + 5;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_time, $str_sub_temp);
					$sem_all_data_got++;
					//echo "time="; echo $str_sub_temp; echo " ";
				}
				if (strpos($line_file,'DATE') !== false) {
					$position1 = strpos($line_file,'DATE=') + 5;
					$position2 = strpos($line_file,' ', $position1);
					$str_sub_temp = substr($line_file, $position1, ($position2 - $position1));
					array_push($array_date, $str_sub_temp);
					$sem_all_data_got++; 
					//echo "date="; echo $str_sub_temp; echo " ";
				}
				//echo " ("; echo $sem_all_data_got; echo ")<br>";
				if($sem_all_data_got==7){
					$cont_data_found++;
				}
				$sem_all_data_got=0;
	...
	echo '    <div style="width:' . $size_px_canvas . 'px;">';
	echo '        <canvas id="' . $id_name_canvas0 . '"></canvas>';
	echo '    </div>';

	//including library for the chart	
	//echo '    <script src="./lib/Chart.min.js"></script>';
	include_script_js_chart($id_name_canvas0);
	echo '    <script src="./lib/utils.js"></script>';
	
	
	//building the data to inser into the chart
		//generating the script for the X
		echo '<script>';
		echo 'var MONTHS = [';
		//"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"
		
		$cont = 0; 
		while($cont<$cont_data_found){
			echo '"';
			echo $array_time[$cont];
			//echo ' ';
			//echo $array_date[$cont];
			echo '"';
			//echo return_str_with_formula_set($array_data[$cont], $array_input_formula_to_show[0], 1);
			$cont++;
			
			if($cont<$cont_data_found)
				echo ',';
		}
		...
		echo '<script>';
		echo 'var config'; echo $id_name_canvas0; echo  ' = {';
        echo "    type: 'line',";
        echo '    data: {';
        echo '        labels: [';
		...
		while($cont<$cont_data_found){
			echo '"';
			if($sem_dislay_time==1)
				echo $array_time[$cont];
			if($sem_dislay_date==1){
				if($sem_dislay_time==1) echo " ";
				echo $array_date[$cont];
			}
			if($sem_dislay_time==0 && $sem_dislay_date==0) echo $cont;
			echo '"';
			//echo return_str_with_formula_set($array_data[$cont], $array_input_formula_to_show[0], 1);
			$cont++;
		...
		//FIRST DATA SET:
		if($sem_dislay_data0==1){
        echo '{ label: "'; echo return_str_with_formula_name($array_input_formula_to_show[0]); echo '",';
        echo '            backgroundColor: window.chartColors.red,';
        echo '            borderColor: window.chartColors.red,';
        echo '            data: [';
		$cont = 0; 
		while($cont<$cont_data_found){
			echo return_num_with_formula_set($array_data0[$cont], $array_input_formula_to_show[0]);
			$cont++;
			if($cont<$cont_data_found)
				echo ',';
		}
        ...
        //SECOND DATA SET:
		if($sem_dislay_data1==1){
		if($sem_dislay_data0==1) echo ', ';
        echo '{ label: "'; echo return_str_with_formula_name($array_input_formula_to_show[1]); echo '",';
        echo '            fill: false,';
		...
		$cont = 0; 
		while($cont<$cont_data_found){
			echo return_num_with_formula_set($array_data1[$cont], $array_input_formula_to_show[1]);;
			$cont++;
		...
		//THIRD DATA SET:
		if($sem_dislay_data2==1){
		if($sem_dislay_data0==1 || $sem_dislay_data1==1) echo ', ';
        echo '{ label: "'; echo return_str_with_formula_name($array_input_formula_to_show[2]); echo '",';
        echo '            fill: false,';
		...
		$cont = 0; 
		while($cont<$cont_data_found){
			echo return_num_with_formula_set($array_data2[$cont], $array_input_formula_to_show[2]);
			$cont++;
		...
		//FOURTH DATA SET:
		if($sem_dislay_data3==1){
		if($sem_dislay_data0==1 || $sem_dislay_data1==1 || $sem_dislay_data2==1) echo ', ';
        echo '{ label: "'; echo return_str_with_formula_name($array_input_formula_to_show[3]); echo '",';
        echo '            fill: false,';
        echo '            backgroundColor: window.chartColors.purple,';
        echo '            borderColor: window.chartColors.purple,';
        echo '            data: [';
		$cont = 0; 
		while($cont<$cont_data_found){
			echo return_num_with_formula_set($array_data3[$cont], $array_input_formula_to_show[3]);
			$cont++;
		...
		//LAST DATA SET:
		if($sem_dislay_data4==1){
		if($sem_dislay_data0==1 || $sem_dislay_data1==1 || $sem_dislay_data2==1 || $sem_dislay_data3==1) echo ', ';
        echo '{ label: "'; echo return_str_with_formula_name($array_input_formula_to_show[4]); echo '",';
        echo '            fill: false,';
		...
		$cont = 0; 
		while($cont<$cont_data_found){
			echo return_num_with_formula_set($array_data4[$cont], $array_input_formula_to_show[4]);
			$cont++;
        ...
        echo '    var ctx'; echo $id_name_canvas0; echo  ' = document.getElementById("'; echo $id_name_canvas0; echo  '").getContext("2d");';
        echo '    window.myLine = new Chart(ctx'; echo $id_name_canvas0; echo  ', config'; echo $id_name_canvas0; echo  ');';
       // echo '};';
	...
	/*echo '<p align=left>';
	echo '    <div style="width:140%;">';
	echo '        <canvas id="canvas"></canvas>';
	echo '    </div>';
	echo '</p>';
*/
	//echo '    <script src="./lib/data_to_display.js"></script>';
}


function print_7columns_of_data_from_the_file($address_peri, $array_input_formula_to_show, $file_data_path){
	...
	$array_date = array();
		...
		if(file_exists($file_data_path)===TRUE){ 
			...
			fclose($handle_file);
		} //echo $cont_data_found;
		
		 
		 
		 
	//Building the table with all data	 
		echo '<table class="table_peripheral" border=1>';
		echo '<tr class="table_title_field_line">'; 
		
		echo '<td class="td_peripheral" colspan=1> NUM REC </td>';
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[0]) . '</td>'; 
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[1]) . '</td>'; 
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[2]) . '</td>';
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[3]) . '</td>';
		echo '<td class="td_peripheral" colspan=1> ' . return_str_with_formula_name($array_input_formula_to_show[4]) . '</td>';
		echo '<td class="td_peripheral" colspan=1> TIME </td>';
		echo '<td class="td_peripheral" colspan=1> DATE </td>';
	...
	//printing datas
		...
		while($cont>=0){
			...
			echo '<td class="td_peripheral" align=center >';
			echo return_str_with_formula_set($array_data0[$cont], $array_input_formula_to_show[0], 1);
			echo "<br>";
			//echo "<br>"; echo $array_input_formula_to_show[$num_input_saved_into_file]; echo "<br>";
			echo '</td>';
				
			echo '<td class="td_peripheral" align=center >';
			echo return_str_with_formula_set($array_data1[$cont], $array_input_formula_to_show[1], 1);
			echo "<br>";
			echo '</td>';
				
			echo '<td class="td_peripheral" align=center >';
			echo return_str_with_formula_set($array_data2[$cont], $array_input_formula_to_show[2], 1);
			echo "<br>";
			echo '</td>';
				
			echo '<td class="td_peripheral" align=center >';
			echo return_str_with_formula_set($array_data3[$cont], $array_input_formula_to_show[3], 1);
			echo "<br>";
			echo '</td>';
				
			echo '<td class="td_peripheral" align=center >';
			echo return_str_with_formula_set($array_data4[$cont], $array_input_formula_to_show[4], 1);
			echo "<br>";
			echo '</td>';
