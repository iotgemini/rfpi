...
echo '<br><p>Reading settings TIMER ....</p>';
...
if( ($counter < $max_counter)){ //if it is not in time out
		...
		if($data[0]==='O' && $data[1]==='K'){ //if there is no answer about the packet then will send a command again
			...
			if($data[0]==='N' && $data[1]==='O' && $data[2]==='T' && $data[3]==='X'){
				...
				if($address_peri === $address_peri_from_reply /*&& $id_packet === $id_packet_reply*/){ //if it is the packet wanted, thus can save it into the DB
					...
					$timer_enabled = intval($data_radio[8] . $data_radio[9], 16); 
					$timer_ms = intval($data_radio[10] . $data_radio[11], 16) << 8;
					$timer_ms += intval($data_radio[12] . $data_radio[13], 16); 
					//echo $data_radio[12] . $data_radio[13]. ' = '; echo $timer_ms; echo '<br>';
					$timer_SS = intval ($data_radio[14] . $data_radio[15], 16); 
					$timer_MM = intval ($data_radio[16] . $data_radio[17], 16); 
					$timer_HH = intval ($data_radio[18] . $data_radio[19], 16); 
					header('Location: ./timer_functions.php?address_peri='.$address_peri.'&position_id='.$position_id.'&timer_enabled='.$timer_enabled.'&timer_ms='.$timer_ms.'&timer_SS='.$timer_SS.'&timer_MM='.$timer_MM.'&timer_HH='.$timer_HH);
					$reload_this_page = 0;
					...
					if($cont_retry<$max_cont_retry+1){
						...
						echo 'setTimeout("';
						echo "location.href = './cmd_get_timer_setting.php?address_peri=".$address_peri."&position_id=".$position_id."&counter=" . $counter . "&cont_retry=".$cont_retry."&$redirect_page=".$redirect_page."';";
						echo '", 10); ';
						echo '</script>';
					}else{
						...
						echo '<script type="text/javascript">';
						echo "setTimeout('";
						echo 'location.href = "./timer_functions.php?address_peri='.$address_peri.'&position_id='.$position_id.'";';
						echo "', 3000);";
						echo '</script>';
					}
	...
	if($reload_this_page == 1){
		echo '<script type="text/javascript">';
		echo 'setTimeout("';
		echo "location.href = './read_fifo_timer_setting.php?address_peri=".$address_peri."&position_id=".$position_id."&counter=" . $counter ."&cont_retry=" . $cont_retry . "&redirect_page=".$redirect_page."';";
		echo '", 25); ';
		echo '</script>';
	}
