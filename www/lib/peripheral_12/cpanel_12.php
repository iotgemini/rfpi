...
//		function peripheral_12($id, $idperipheral, $name, $address_peri, $numInput, $numOutput, $arrayNameInput, $arrayStatusInput, $arrayNameOutput, $arrayStatusOutput);
...
//		Specific library for the Peri_12
		include './lib/peripheral_12/peri_12_lib.php';  
...
function peripheral_12($id, $idperipheral, $name, $address_peri, $numInput, $numOutput, $arrayNameInput, $arrayStatusInput, $arrayNameOutput, $arrayStatusOutput, $num_special_functions_peri, $fw_version_peri){
	...
	echo '<link rel="stylesheet" href="' . DIRECTORY_CSS_PERI_12 . 'peripheral.css" type="text/css" >';
	...
	if($arrayStatusInput[0]==-1){
		echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'ant_grey.png" class="img_ant" alt="'.DEFINE_lang_msg_no_communication.'"> ';
	}else{
		echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'ant_green.png" class="img_ant" alt="'.$lang_msg_connected.'"> ';
	}
	echo '<br>';
	echo '<br>';
	...
	while ($l<$numInput) { 
		...
		if(($l==0 || $l==1 || $l==2 || $l==3) && $array_input_to_show[$l]==1){
			...
			if($arrayStatusInput[$l]==1){
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'led_green.png" class="img_led" alt="'.$lang_msg_on.'"> ';
				echo '= 1';
			}else if($arrayStatusInput[$l]==0){
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'led_red.png" class="img_led" alt="'.$lang_msg_off.'"> ';
				echo '= 0';
			}else{
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'led_grey.png" class="img_led" alt="'.DEFINE_lang_msg_no_communication.'"> ';
				echo '= &#63;';
			}
			echo '<br>';
		...
		/*}else if($l==1 && $array_input_to_show[$l]==1){
			...
			if($arrayStatusInput[$l]>=0){
				...
				$temperature = temperature_MCP9701_from_8bit_value_peri_12($temperature);
		...
		*/	
		/*
		}else if($l==4 && $array_input_to_show[$l]==1){
			
			if($arrayStatusInput[$l]>=0){
				if($array_input_formula_to_show[$l]==1){
					echo str_temperature_pyrometer_peri_12($arrayStatusInput[$l]);
				}else{
					echo '<br>&nbspADC0= ';
					echo  str_voltage_0to10V_from_8bit_value_peri_12($arrayStatusInput[$l]);
					echo '&nbspV&nbsp'; //°C
				}
			}else{
				echo ' '.DEFINE_lang_msg_no_communication.' ';
		...
		}else if($l==5 && $array_input_to_show[$l]==1){
	...
	while ($l<$numOutput) {
		...
		if($array_output_to_show[$l]==1){
			if($arrayStatusOutput[$l]==0){
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'on.png" onclick="change' . $id . '_' . $l . '(1)"  class="img_button_square_enable" alt="'.$lang_msg_turn_on.'"> ';
				echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'switch_off.png" onclick="change' . $id . '_' . $l . '(1)"  class="img_switch_off" alt="'.$lang_msg_turn_on.'"> ';
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'off.png"  class="img_button_square_disable" alt="'.$lang_msg_turn_off.'"> ';
			}else{
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'on.png" class="img_button_square_disable" alt="'.$lang_msg_turn_on.'"> ';
				//echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'off.png" onclick="change' . $id . '_' . $l . '(0)"  class="img_button_square_enable" alt="'.$lang_msg_turn_off.'"> ';
				echo '<img src="' . DIRECTORY_IMG_PERI_12 . 'switch_on.png" onclick="change' . $id . '_' . $l . '(0)"  class="img_switch_on" alt="'.$lang_msg_turn_off.'"> ';
			}
	...
	echo '<form name="peri_12_btn_timer1_functions_'.$id.'" action="./lib/peripheral_12/lib/cmd_get_settings.php" method=GET>';
	...
	echo '<input type=submit value="'.$lang_btn_timer.'1" class="btn_functions">';
	echo '</form>';
	}
	
	//Button to page TIMER Relay2 Functions
	if($array_function_to_show[0]==1){
	echo '<form name="peri_12_btn_timer2_functions_'.$id.'" action="./lib/peripheral_12/lib/cmd_get_settings.php" method=GET>';
	echo '<input type=hidden name="position_id" value="'.$id.'">';
	echo '<input type=hidden name="address_peri" value="'.$address_peri.'">';
	...
	echo '<input type=hidden name="TAG3" value="'.$str_TAG3.'">';		//fourth parameter
	echo '<input type=hidden name="page_to_show_data" value="show_settings_fifo_timer.php">';
	echo '<input type=submit value="'.$lang_btn_timer.'2" class="btn_functions">';
	echo '</form>';
	}
	...
	echo '<form name="peri_12_btn_input_functions_'.$id.'" action="./lib/peripheral_12/cmd_get_input_setting.php" method=GET>';
	echo '<input type=hidden name="position_id" value="'.$id.'">';
	echo '<input type=hidden name="address_peri" value="'.$address_peri.'">';
	echo '<input type=submit value="'.$lang_btn_input.'" class="btn_functions">';
	echo '</form>';
	...
	if($array_function_to_show[2]==1){
	echo '<form name="peri_12_btn_rtc_functions_'.$id.'" action="./lib/peripheral_12/cmd_get_settings.php" method=GET>';
	echo '<input type=hidden name="position_id" value="'.$id.'">';
	echo '<input type=hidden name="address_peri" value="'.$address_peri.'">';
